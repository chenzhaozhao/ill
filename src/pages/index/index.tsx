...
const bizType = "012502",
  ...
  userId = "yhzj_001",
  token =
    "eyJhbGciOiJIUzI1NiJ9.eyJhcHBJZCI6IjYxOWM1NTRhODM3ZmVlM2E3MjgwMGM3NyIsImlhdCI6MTY2MDYyMDA3NywiZXhwIjoyMjY1NDIwMDc3fQ.QvglJm8UCr_W9uWeXOd3rZNoMSN9DHE9v6IvDSDJK3Y",
...
const Index = () => {
    ...
    (async () => {
      if (codes.length > 0 && step === 1) {
        const {
          ...
          url: `https://api3.jiankangyouyi.com/assessment/v5/api/permission`,
          method: "POST",
          params: { bizType },
        ...
        const {
          data: {
            data: { questionnaire:{questionnaireItems,questionCount} }
          }
        } = await axios({
          url: `https://api3.jiankangyouyi.com/assessment/v3/api/health-center/questionnaire`,
          method: "POST",
          params: { bizType },
          data: {
            configId,
  ...
  return (
    <View className="main">
      ...
      {((item, step) => {
        if (step <= total) {
          const { questionType, question } = item;
          return (
            <View>
              <View className=" flex mt-1 item-center px-1-2">
                <View><Image src={botImg} className="bot-img"></Image></View>
